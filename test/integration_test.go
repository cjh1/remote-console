...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	rfEmulator0, err := startRedfishEmulator(ctx, s.rfNetwork.Name, "x0c0s0b0", "ssh", &authConfig)
	require.NoError(s.T(), err)
	...
	rfEmulator1, err := startRedfishEmulator(ctx, s.rfNetwork.Name, "x0c0s1b0", "ssh", &keyAuthConfig)
	require.NoError(s.T(), err)
	...
	rfEmulator2, err := startRedfishEmulator(ctx, s.rfNetwork.Name, "x0c0s2b0", "ipmi", &keyAuthConfig)
	require.NoError(s.T(), err)
	...
	require.NoError(s.T(), err)
	err = loadRedfishEndpoints(ctx, smdAPIURL, redfishEndpoints)
	require.NoError(s.T(), err)

	// Start SSH password server
	s.T().Log("Starting SSH password server...")
	sshPasswordServer, err := startSSHPasswordServer(ctx, s.consoleNetwork.Name, "x0c0s0b0", "ADMIN", "ADMIN")
	require.NoError(s.T(), err)
	s.containers["ssh-password"] = sshPasswordServer

	// Start SSH key server
	s.T().Log("Starting SSH key server...")
	sshKeyServer, err := startSSHKeyServer(ctx, s.consoleNetwork.Name, "x0c0s1b0", "ADMIN", publicKey)
	require.NoError(s.T(), err)
	s.containers["ssh-key"] = sshKeyServer

	// Start IPMI server
	s.T().Log("Starting IPMI server...")
	ipmiServer, err := startIPMIServer(ctx, s.consoleNetwork.Name, "x0c0s2b0")
	require.NoError(s.T(), err)
...
func (s *IntegrationTestSuite) TestDynamicConsoleDiscovery() {
	...
	err = loadRedfishEndpoints(ctx, smdAPIURL, []redfishEndpoint{{
		Host:     newNodeID,
		Username: "ADMIN",
		Password: "ADMIN",
	}})
	...
	smdAPIURL, err = getSMDAPIURL(ctx, s.containers["smd"])
	s.Require().NoError(err)
	err = deleteRedfishEndpoint(ctx, smdAPIURL, newNodeID)
